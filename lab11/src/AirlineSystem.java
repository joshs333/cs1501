  ...
  private void shortestHops() {
    if(G == null){
      ...
      if(!G.marked[destination]){
        System.out.println("There is not route from " + cityNames[source]
        ...
        for(int place = destination; place != -1; place = G.edgeTo[place]) {
            System.out.println(cityNames[place]);
            hops.insert(0, " " + cityNames[place]);
        }
        System.out.println("The shortest route from " + cityNames[source]
                              + " to " + cityNames[destination] + " has " + G.distTo[destination] + " hop(s):" + hops.toString());
      }
      System.out.print("Please press ENTER to continue ...");
      scan.nextLine();
    }
      ...
      while (!q.isEmpty()) {
        ...
        for (DirectedEdge w : adj(v)) {
          if (!marked[w.to()]) {
            ...
            if(w.from() == v) {
                System.out.println("Got to " + w.to() + " from " + v);
                marked[w.to()] = true;
